...
import { Harvest, Unit } from '../../../types/types';
...
export default function EditHarvestScreen() {
      ...
      const loadHarvest = async () => {
        try {
          ...
          if (harvestData) {
            ...
            setPlantName(harvestData.plantName);
            setQuantity(String(harvestData.quantity));
            setUnit(harvestData.unit);
            setDate(new Date(harvestData.date));
            setNotes(harvestData.notes || '');

            
            // Load garden name
            if (harvestData.gardenId) {
              const garden = await gardenService.getGarden(harvestData.gardenId);
              if (garden) {
  ...
  const handleSubmit = async () => {
    if (!validate() || !id || !harvest) return;
    
    setIsLoading(true);
    try {
      await harvestService.updateHarvest(id, {
      ...
      Alert.alert('Error', 'Failed to update harvest. Please try again.');
      setIsLoading(false);
    }
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('Error', 'Failed to delete harvest. Please try again.');
            }
          },
        },
      ]
      ...
      <Header
        ...
        rightAction={
          <TouchableOpacity onPress={handleDelete} style={styles.deleteButton}>
            <MaterialIcons name="delete" size={24} color={theme.colors.textSecondary} />
          </TouchableOpacity>
        }
      />
